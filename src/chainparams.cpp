...
public:
    CMainParams() {
        ...
        const char* pszTimestamp = "Nasci Homines Postumum";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNewvout1nValue = 66/2;
        txNewvout2nValue = 1999999 * COIN;
...
public:
    CTestNetParams() {
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        // vSeeds.push_back(CDNSSeedData("illuminaticoin.test", "test.illuminaticoin.org"));
