IlluminatiCoin-Qt build instructions for Mac OS X
==========================================

This document build and configuration instructions for building IlluminatiCoin-Qt (Qt4 GUI for IlluminatiCoin) for OS X.  A illuminaticoind binary is not included in the IlluminatiCoin-Qt.app bundle but can be built using the instructions in doc/build-osx.md.
...
### Modify illuminaticoin-qt.pro

Modify illuminaticoin-qt.pro since it has options set to build for a Windows environment.
...
Generate a Qt Makefile
        ...
        qmake USE_UPNP=1 USE_QRCODE=1 BOOST_LIB_PATH=/usr/local/lib BOOST_INCLUDE_PATH=/usr/local/include BDB_LIB_PATH=/usr/local/opt/berkeley-db4/lib BDB_INCLUDE_PATH=/usr/local/opt/berkeley-db4/include illuminaticoin-qt.pro
<<<<<<< HEAD
...
During the process, the disk image window will pop up briefly where the settings are applied. This is normal, please do not interfere with it and allow the window to disappear.
        ...
        /usr/local/bin/python contrib/macdeploy/macdeployqtplus IlluminatiCoin-Qt.app -add-qt-tr $T -dmg -fancy contrib/macdeploy/fancy.plist

You may receive this error:

       112:116: execution error: Finder got an error: Can’t get disk "IlluminatiCoin-Qt". (-1728)
       Error running osascript.

If you do, remove the temporarily created dmg file and run the last command again

       rm IlluminatiCoin-Qt.temp.dmg

When finished, it will produce a beautifully packaged Apple Disk Image `IlluminatiCoin-Qt.dmg`.
